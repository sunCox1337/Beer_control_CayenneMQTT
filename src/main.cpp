#include <Arduino.h>
  ...
  // WiFi network info.
  char ssid[] = "Vika"; // Имя WI FI точки доступа
  char wifiPassword[] = "22062007"; // Пароль 
  ...
  void setup() {
    ...
    digitalWrite(relayPin1, HIGH); // Состояние пина низкое 0
  ...
  // Условия для датчиков температуры и включения реле
    if (temp_0 > Warring) { // Если температура > переменной Warring включить реле
      digitalWrite(relayPin1, LOW);
    }
    else {
  digitalWrite(relayPin1, HIGH); // Выключить реле
  }
  }